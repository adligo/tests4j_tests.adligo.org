...
public class AssertContainsNullFailsTrial extends ApiTrial {

	@Test
	public void testAssertContainsNullFails() {
		assertContains(null, "set");
	}
	...
	public static void runTestDelegate(I_Asserts asserts) {
		...
		asserts.assertEquals(AssertContainsNullFailsTrial.class.getName(), 
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (AssertContainsNullFailsTrial.class.getName().equals(className)) {
				break;
			}
		}
		
		asserts.assertEquals(AssertContainsNullFailsTrial.class.getName(), topElement.getClassName());
