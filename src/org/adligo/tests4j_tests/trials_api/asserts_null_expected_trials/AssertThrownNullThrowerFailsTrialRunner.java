...
public class AssertThrownNullThrowerFailsTrialRunner extends ApiTrial {
	private boolean called = false;
	@Test
	public void testAssertThrownNull() {
		called = false;
		assertFalse(called);
		assertThrown(new ExpectedThrownData(new RuntimeException("hey")), null);
		assertTrue(called);
	}
	
	public static void runTestDelegate(I_Asserts asserts) throws Exception  {
		ExpectedFailureRunner runner = new ExpectedFailureRunner();
		runner.run(AssertThrownNullThrowerFailsTrialRunner.class);
		...
		asserts.assertEquals(AssertThrownNullThrowerFailsTrialRunner.class.getName(), 
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (AssertThrownNullThrowerFailsTrialRunner.class.getName().equals(className)) {
				break;
			}
		}
		
		asserts.assertEquals(AssertThrownNullThrowerFailsTrialRunner.class.getName(), topElement.getClassName());
