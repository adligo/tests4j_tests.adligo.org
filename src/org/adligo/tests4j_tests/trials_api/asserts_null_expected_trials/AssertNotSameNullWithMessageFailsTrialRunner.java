...
public class AssertNotSameNullWithMessageFailsTrialRunner extends ApiTrial {
	...
	public void testAssertNotSameWithMessage() {
		assertNotSame(TEST_ASSERT_FAILS_MESSAGE, null, null);
	}
	
	public static void runTestDelegate(I_Asserts asserts) throws Exception  {
		ExpectedFailureRunner runner = new ExpectedFailureRunner();
		runner.run(AssertNotSameNullWithMessageFailsTrialRunner.class);
		...
		asserts.assertEquals(AssertNotSameNullWithMessageFailsTrialRunner.class.getName(), 
		...
		asserts.assertEquals("testAssertNotSameWithMessage", testMeta.getTestName());
		...
		asserts.assertEquals("testAssertNotSameWithMessage", testResult.getName());
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (AssertNotSameNullWithMessageFailsTrialRunner.class.getName().equals(className)) {
				break;
			}
		}
		
		asserts.assertEquals(AssertNotSameNullWithMessageFailsTrialRunner.class.getName(), topElement.getClassName());
