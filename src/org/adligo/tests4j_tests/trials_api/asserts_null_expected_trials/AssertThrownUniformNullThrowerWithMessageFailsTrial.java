...
public class AssertThrownUniformNullThrowerWithMessageFailsTrial extends ApiTrial {
	...
	public static void runTestDelegate(I_Asserts asserts) {
		...
		for (int i = 0; i < elements.length; i++) {
			...
			if (AssertThrownUniformNullThrowerWithMessageFailsTrial.class.getName().equals(className)) {
				break;
			}
		}
		
		asserts.assertEquals(AssertThrownUniformNullThrowerWithMessageFailsTrial.class.getName(), topElement.getClassName());
		...
		asserts.assertUniform(new IllegalArgumentException(
				new Tests4J_AssertionResultMessages().getIThrowerIsRequired()),
				exception);
	}
