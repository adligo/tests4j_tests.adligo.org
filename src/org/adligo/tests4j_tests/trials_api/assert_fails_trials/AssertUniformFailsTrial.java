...
public class AssertUniformFailsTrial extends ApiTrial {

	@Test
	public void testAssertUniform() {
		assertUniform("this", "thi");
	}
	
	public static void runTestDelegate(I_Asserts asserts) {
		ExpectedFailureRunner runner = new ExpectedFailureRunner();
		runner.run(AssertUniformFailsTrial.class);
		...
		asserts.assertEquals(AssertUniformFailsTrial.class.getName(), 
		...
		asserts.assertEquals("testAssertUniform", testMeta.getTestName());
		...
		asserts.assertEquals("testAssertUniform", testResult.getName());
		...
		asserts.assertEquals(new Tests4J_AssertionResultMessages().getTheObjectsShouldBeUniform(), testFailure.getMessage());
		...
		asserts.assertEquals(AssertUniformFailsTrial.class.getName(), topElement.getClassName());
		asserts.assertEquals("testAssertUniform", topElement.getMethodName());
