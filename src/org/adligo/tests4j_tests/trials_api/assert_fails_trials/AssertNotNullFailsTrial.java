...
public class AssertNotNullFailsTrial extends ApiTrial {
	...
	public static void runTestDelegate(I_Asserts asserts) {
		...
		runner.run(AssertNotNullFailsTrial.class);
		...
		asserts.assertEquals(AssertNotNullFailsTrial.class.getName(), 
		...
		asserts.assertEquals("testAssertNotNull", testMeta.getTestName());
		...
		asserts.assertEquals("testAssertNotNull", testResult.getName());
		...
		asserts.assertEquals(new Tests4J_AssertionResultMessages().getTheValueShould_NOT_BeNull(), testFailure.getMessage());
		...
		asserts.assertEquals(AssertNotNullFailsTrial.class.getName(), topElement.getClassName());
		asserts.assertEquals("testAssertNotNull", topElement.getMethodName());
