...
public class CPDT_Assert_Simple extends TrialDelegate {
	...
	public void delegateObjectOnlyParents(Class<?> clazz) throws Exception {
		...
		assertEquals(2, ccbClassLoader.getCacheSize());
	}
	...
	public void delegate003_MockException() throws Exception {
		...
		assertTrue(ccbClassLoader.hasCache(Object.class.getName()));
		assertTrue(ccbClassLoader.hasCache(Throwable.class.getName()));
		assertTrue(ccbClassLoader.hasCache(Exception.class.getName()));
		assertTrue(ccbClassLoader.hasCache(Serializable.class.getName()));
		assertTrue(ccbClassLoader.hasCache(clazz.getName()));
		
		assertEquals(5, ccbClassLoader.getCacheSize());
	}
